"""
Gobuster directory scan import: create/find host and port, add parent Evidence
"Web Directories" and child Evidence per path with caption "{full_url} [gobuster]".
"""
...
from app.services.gobuster_parser import (
    GOBUSTER_SOURCE,
    GobusterParseResult,
    parse_gobuster,
)
...
def _find_or_create_host(
    ...
    if not host_str:
        raise ValueError("Gobuster parse result has no host")
...
def _build_parent_notes_md(parse_result: GobusterParseResult) -> str:
...
def run_gobuster_import(
    ...
    """
    Parse Gobuster output and create parent "Web Directories" Evidence and
    one child Evidence per path with caption "{full_url} [gobuster]".
    ...
    parse_result = parse_gobuster(content, filename)
    ...
    # Child Evidence per path: caption = "{full_url} [gobuster]"
    for p in parse_result.paths:
        child_caption = f"{p.full_url} [gobuster]"
    ...
    log_audit(
        ...
        project_id=project_id,
        user_id=user_id,
        action_type="gobuster_import_completed",
        record_type="project",
