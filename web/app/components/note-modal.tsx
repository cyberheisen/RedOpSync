...
export function NoteModal({ hostIp, hostDnsName, note, onClose, onSave }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: "0 0 16px", fontSize: "1.25rem" }}>{note ? "Edit note" : "Add note"}</h2>
        <p style={{ margin: "0 0 16px", fontSize: 14, color: "var(--text-muted)" }}>
          Host: {hostIp}{hostDnsName ? ` (${hostDnsName})` : ""}
        </p>
        ...
        <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexShrink: 0 }}>
            ...
            <button type="submit" className="theme-btn theme-btn-primary" disabled={saving}>
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
        </form>
